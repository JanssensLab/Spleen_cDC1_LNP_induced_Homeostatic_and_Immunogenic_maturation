...
## Muscat script for comparing between conditions and maturation stages part 2 RNA
...
## New order 2024
seuratObj_subset$annotated_clusters_Muscat_v4<-seuratObj_subset$annotated_clusters_Muscat_v3
seuratObj_subset$annotated_clusters_Muscat_v4<-factor(as.character(seuratObj_subset$annotated_clusters_Muscat_v4), 
                                                      levels = c("Steady_state_Immature_cDC1s","Steady_state_Early_mature_cDC1s","Steady_state_Late_mature_cDC1s",
                                                                 "eLNPs_2h_Immature_cDC1s","eLNPs_2h_Early_mature_cDC1s","eLNPs_8h_Late_mature_cDC1s", 
                                                                 "pIC_alone_2h_Immature_cDC1s","pIC_alone_2h_Early_mature_cDC1s","pIC_alone_8h_Late_mature_cDC1s",
                                                                 "pIC_LNPs_2h_Immature_cDC1s","pIC_LNPs_2h_Early_mature_cDC1s","pIC_LNPs_8h_Late_mature_cDC1s",
...
sce$group_id<-sce$annotated_clusters_Muscat_v4 #2024 MDS plot
...
dir.create(paste0(sampleFolder,"results/Muscat_RNA_Thesis_Victor/"))

Comparison<-"DS_RNA_thesis_Victor"

png(file=paste0(sampleFolder,"results/Muscat_RNA_Thesis_Victor/meanVariancePlot_subset_",Comparison,".png"),width = 3500, height = 2000, res = 300)
...
sce_subset@colData$cluster_id<-sce_subset@colData$group_id
pb_subset <- aggregateData(sce_subset,
                           assay = "counts", fun = "sum",
                           by = c("cluster_id", "sample_id"))
(pb_mds_subset <- pbMDS(pb_subset) + scale_shape_manual(values=1:nlevels(pb_subset$group_id)))

pdf(file=paste0(sampleFolder,"results/Muscat_RNA_Thesis_Victor/meanVariancePlot_subset_paper_",Comparison,".pdf"),width = 15, height = 8)
pb_mds_subset + scale_color_manual(values = c("cadetblue1",brewer.pal(10, "Paired")[1:4],brewer.pal(10, "Paired")[9:10],brewer.pal(10, "Paired")[5:8]))
dev.off()

## Extra version 2024: have symbols make sense per group!
pdf(file=paste0(sampleFolder,"results/Muscat_RNA_Thesis_Victor/meanVariancePlot_subset_paper_v2_",Comparison,".pdf"),width = 8, height = 6)
pb_mds_subset + scale_color_manual(values = c("cadetblue1",brewer.pal(10, "Paired")[1:4],brewer.pal(10, "Paired")[9:10],brewer.pal(10, "Paired")[5:8])) +
  scale_shape_manual(values=c(1,10,13,0,12,5,9,4,8,2,6))
dev.off()
...
for (i in 1:length(res$table)){
  ...
  write.xlsx(tbl, paste0(sampleFolder,"results/Muscat_RNA_Thesis_Victor/tbl_full_muscat_DESeq2_new_",Comparison,"_",Comp,"_",sampleName,".xlsx"))
  ...
  write.xlsx(tbl_fil, paste0(sampleFolder,"results/Muscat_RNA_Thesis_Victor/tbl_fil_muscat_DESeq2_new_",Comparison,"_",Comp,"_",sampleName,".xlsx"))
  ...
  write.xlsx(tbl_fil_clint, paste0(sampleFolder,"results/Muscat_RNA_Thesis_Victor/tbl_fil_muscat_DESeq2_new_",Comparison,"_",Comp,"_",sampleName,"_clint.xlsx"))
  
}

### Extra ###
tbl_freq<- resDS(sce, res, frq = TRUE)
write.xlsx(tbl_freq, paste0(sampleFolder,"results/Muscat_RNA_Thesis_Victor/tbl_full_freq_muscat_DESeq2_new_",Comparison,"_",sampleName,".xlsx"))
...
for (i in 1:length(res$table)){
  ...
  pdf(file=paste0(sampleFolder,"results/Muscat_RNA_Thesis_Victor/Heatmap_Overview_DESeq2_new_normalized_",Comparison,"_",Comp,".pdf"), width = 10, height = 8)
  print(H2)
  dev.off()
